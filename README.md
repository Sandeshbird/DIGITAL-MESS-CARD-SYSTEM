...
A short plan (3–6 steps) — what files change, migrations, tests.
The exact code files (complete, copy-paste ready) — filename header on top of each code block.
SQL migration files.
Unit test files.
A CHANGELOG.md entry describing the changes.
A short verification checklist with commands to run locally (curl examples, composer/phpunit commands, docker commands).
